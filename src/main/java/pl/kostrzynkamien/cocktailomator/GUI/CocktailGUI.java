...
public class CocktailGUI extends VerticalLayout {
    ...
    public CocktailGUI(CocktailController cocktailController, CocktailService cocktailService, FacebookShareGui facebookShareGui) {
        ...
        buttonsClickListeners(cocktailController, cocktailService);
        layoutInit();
    }
    ...
    private void buttonsClickListeners(CocktailController cocktailController, CocktailService cocktailService) {
    ...
    private void createAPost(Grid<FavouriteCocktail> gridFacebookShare) {
        if (gridFacebookShare.getSelectionModel().getFirstSelectedItem().isPresent()) {
            ...
            try {
                Runtime.getRuntime().exec("rundll32 url.dll,FileProtocolHandler " + "https://www.facebook.com/sharer.php?u=cocktailmator.herokuapp.com/share");
            } catch (Exception e) {
    ...
    private void CreateNewFromDialog(CocktailService cocktailService, TextField drinkName, TextField category, TextField genre, TextField glass, TextField instructions, TextField drinkThumb,
        ...
        try {
            if (!drinkName.getValue().isEmpty() && !instructions.getValue().isEmpty() && !ingredientList.get(0).isEmpty() && !measureList.get(0).isEmpty()) {
                if (drinkThumb.isEmpty())
                    drinkThumb.setValue("https://image.shutterstock.com/image-vector/big-collection-cocktails-daiquiri-mojito-600w-1331419013.jpg");
                ...
                Notification.show("DrinkName, instructions and at least Ingredient1 and measure1 muss be filled!",
                        5000, Notification.Position.TOP_CENTER);
            }
        } catch (Exception e) {
            Notification.show("Something went wrong!",
    ...
    private void favouriteAdder(CocktailService cocktailService) {
        if (cocktailService.getAllFavouriteCocktails().stream().anyMatch(element -> element.getStrDrink().equals(specificAndRandomGrid.getSelectionModel().getFirstSelectedItem().get().getStrDrink())))
            Notification.show("This cocktail is already in your favourites",
                    5000, Notification.Position.TOP_CENTER);
        else if (specificAndRandomGrid.getSelectionModel().getFirstSelectedItem().isPresent()) {
